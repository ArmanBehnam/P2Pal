...
public:
    ...
    void initialize(quint16 port, const QString& connectTo = QString(), bool noForward = false);
    quint16 getPort() const;

public slots:
    void onMessageSent(const QString& message);
    void onReceivedMessage(const QVariantMap& message, const QString& senderIP, quint16 senderPort);
    void onConnectionAccepted(const QString& ipAddress, quint16 port);
    void onConnectionRequested(const QString& ipAddress, quint16 port);
    void onAvailableNodesChanged(const QStringList& nodes);
    void onPrivateMessageRequested(const QString& destNodeId, const QString& message);
...
private:
    ...
    void sendPrivateMessage(const QString& destNodeId, const QString& message);
};
